...
After transcription, a content worker uses OpenRouter structured output with configured fallbacks to generate a summary, outline, key thoughts, reflection questions, and follow-up post drafts. Generated drafts remain unpublished until an administrator approves them. The Bible assistant uses the same provider chain; urgent safety routing happens locally before any provider request.
...
Large sermons can be submitted with `/sermon_link HTTPS_URL`. Public linked files are accepted up to `SERMON_MAX_LINK_FILE_SIZE_BYTES` (500 MB by default), normalized to 16 kHz mono OGG/Opus with `ffmpeg`, split into one-hour segments, transcribed sequentially by Groq, and joined before structured AI analysis. The Raspberry Pi deployment also accepts direct Telegram audio up to 500 MB through the Local Bot API; deployments using Telegram's hosted Bot API remain subject to its download limit.

`/sermon_status ID` shows one Russian-language timeline for receipt, download, audio preparation, transcription, analysis, moderation, image generation, and delivery. Terminal failures are converted into safe explanations and concrete recovery steps without exposing provider responses, filesystem paths, transcript text, or secrets. Newly accepted group audio includes a **Проверить обработку** button, and the status view can refresh or regenerate eligible AI materials without console-style commands.
