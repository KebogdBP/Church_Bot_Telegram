...
export class RetentionService {
  ...
  public async preview(chatId: string): Promise<RetentionPreview | null> {
    ...
    const [audio, transcripts, prayers] = await this.prisma.$transaction([
      this.prisma.sermon.count({ where: { churchGroupId: context.id, storedPath: { not: null }, createdAt: { lt: context.audioBefore } } }),
      this.prisma.sermon.count({ where: { churchGroupId: context.id, transcript: { not: null }, createdAt: { lt: context.transcriptBefore } } }),
  ...
  public async execute(chatId: string): Promise<RetentionPreview | null> {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const audioRows = await tx.sermon.findMany({ where: { churchGroupId: context.id, storedPath: { not: null }, createdAt: { lt: context.audioBefore } }, select: { id: true, storedPath: true } });
      for (const row of audioRows) if (row.storedPath) await rm(row.storedPath, { force: true });
      ...
      const transcripts = (await tx.sermon.updateMany({ where: { churchGroupId: context.id, transcript: { not: null }, createdAt: { lt: context.transcriptBefore } }, data: { transcript: null } })).count;
