...
export class OpenAIBibleAnswerProvider implements BibleAnswerProvider {
  ...
  public async answer(question: string, churchContext?: string) {
    const response = await this.request(`${this.baseUrl}/responses`, { method: 'POST', headers: { Authorization: `Bearer ${this.apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({
      ...
      input: question,
      text: { format: { type: 'json_schema', name: 'bible_answer', strict: true, schema: { type: 'object', additionalProperties: false, properties: { answer: { type: 'string', maxLength: 3200 }, bibleReferences: { type: 'array', items: { type: 'string', maxLength: 80 }, maxItems: 10 }, needsPastor: { type: 'boolean' }, category: { type: 'string', maxLength: 50 } }, required: ['answer', 'bibleReferences', 'needsPastor', 'category'] } } },
    }) });
