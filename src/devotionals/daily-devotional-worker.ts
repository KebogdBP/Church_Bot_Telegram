...
export class DailyDevotionalWorker {
  ...
  public constructor(private readonly options: { prisma: PrismaClient; provider: OpenRouterDevotionalProvider; sender: MessageSender; logger: Pick<FastifyBaseLogger, 'info' | 'warn' | 'error'>; intervalMs: number; now?: () => Date }) {}
  ...
  public async tick(): Promise<void> {
    ...
    try {
      ...
      for (const group of groups) {
        ...
        await this.options.prisma.dailyDevotional.create({ data: { churchGroupId: group.id, localDate, content: formatDevotional(devotional), availableAt: dueAt.toUTC().toJSDate() } });
      }
      ...
      for (const item of due) {
        ...
        try { await this.options.sender.sendMessage({ chatId: item.churchGroup.telegramChatId, text: item.content }); await this.options.prisma.dailyDevotional.update({ where: { id: item.id }, data: { status: DailyDevotionalStatus.SENT, sentAt: now, lastError: null } }); }
...
function formatDevotional(value: { scriptureReference: string; scriptureText: string; reflection: string; sermonConnection: string; practice: string; prayer: string; question: string }): string {
  return [`<b>Утреннее размышление</b>`, `<b>${escapeHtml(value.scriptureReference)}</b>`, escapeHtml(value.scriptureText), '', `<b>Размышление</b>\n${escapeHtml(value.reflection)}`, `<b>Связь с проповедью</b>\n${escapeHtml(value.sermonConnection)}`, `<b>Практика</b>\n${escapeHtml(value.practice)}`, `<b>Молитва</b>\n${escapeHtml(value.prayer)}`, `<b>Вопрос дня</b>\n${escapeHtml(value.question)}`].join('\n');
}
