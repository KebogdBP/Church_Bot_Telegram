...
const envSchema = z.object({
  ...
  CLOUDFLARE_API_TOKEN: optionalString(z.string().min(1)),
  CLOUDFLARE_ACCOUNT_ID: optionalString(z.string().regex(/^[a-f0-9]{32}$/i)),
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    ai: {
      ...
      cloudflareApiBaseUrl: parsed.CLOUDFLARE_API_BASE_URL.replace(/\/$/, ''),
      cloudflareImageModel: parsed.CLOUDFLARE_IMAGE_MODEL,
