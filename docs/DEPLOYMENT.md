...
2. Create `.env` from `.env.example`. Set unique `POSTGRES_PASSWORD`, `APP_PRIVACY_SECRET`, `TELEGRAM_WEBHOOK_SECRET`, Telegram/OpenAI keys, `APP_ENV=production`, and `TELEGRAM_UPDATE_MODE=webhook`.
