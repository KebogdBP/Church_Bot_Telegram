...
2. Create `.env` from `.env.example`. Set unique `POSTGRES_PASSWORD`, `APP_PRIVACY_SECRET`, `TELEGRAM_WEBHOOK_SECRET`, Telegram, Gemini, Groq, and OpenRouter keys, `APP_ENV=production`, and `TELEGRAM_UPDATE_MODE=webhook`. For generated devotional and sermon images, also set `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`; OpenRouter remains the image fallback.
