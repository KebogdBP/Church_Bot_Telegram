...
- Implemented with group-scoped day limits, database-enforced bounds, PostgreSQL advisory locking, an exact `CONFIRM` command, processing-time timestamps, a durable file-deletion queue, idempotent field clearing/deletion, and privacy-safe audit counts.
